...
# Choose which algorithm to run: "vanilla", "tabpfn", "tabpfn_advanced"
ALGORITHM = "vanilla"  # Change this to run different algorithms
...
algorithm_name = ("Vanilla BO" if ALGORITHM == "vanilla" 
                 else "TabPFN BO")

algorithm_info = ("Bo-Torch Implementation" if ALGORITHM == "vanilla" 
                 else "TabPFN-BoTorch Implementation")
...
def run_optimization(problem_id=1, dimension=2, budget=None, n_runs=5, instance=1, 
                   fit_mode="fit_with_cache", device="auto", memory_saving_mode = False):
    """
    ...
    for run in range(1, n_runs+1):
        ...
        try:
            ...
            run_algorithm_name = ("Vanilla BO" if ALGORITHM == "vanilla" 
                                 else "TabPFN BO")
            
            run_algorithm_info = ("Bo-Torch Implementation" if ALGORITHM == "vanilla" 
                                 else "TabPFN-BoTorch Implementation")
...
def plot_multi_algorithm_comparison(algorithm_data, problem_id, dimension, save_path=None):
    """
    Plot the mean convergence curves of multiple algorithms on a single plot.
    
    Args:
        algorithm_data: Dictionary with algorithm names as keys and convergence data as values
        problem_id: IOH problem ID
        dimension: Problem dimension
    ...
    if save_path:
        fig.savefig(save_path, format='png', dpi=300, bbox_inches='tight', pad_inches=0.5)
        print(f"Multi-algorithm comparison plot saved to {save_path}")
    else:
        plt.show()
...
if __name__ == "__main__":
    try:
        ...
        dimension = 2
        n_runs = 2
        budget = 10*dimension+50  # Increased from 100 to allow for optimization iterations after DoE
        instance = 1
        ...
        # 3. Then change to ALGORITHM = "tabpfn_advanced" to test advanced BarDistribution preservation
        # 4. To test different acquisition behaviors, change ACQUISITION_FUNCTION to one of:
        ...
        print(f"Using algorithm: {ALGORITHM} on device: cpu")  # Force CPU
        ...
        # Check for data from all three algorithms and create comparison plots
        vanilla_folder = "bo-experiment-vanilla"
        tabpfn_folder = "bo-experiment-tabpfn"
        tabpfn_advanced_folder = "bo-experiment-tabpfn_advanced"
        
        available_algorithms = []
        algorithm_data = {}
        
        # Check for vanilla BO data
        if Path(vanilla_folder).exists():
            vanilla_data = []
            for run in range(1, n_runs + 1):
            ...
            if vanilla_data:
                available_algorithms.append("Vanilla BO")
                algorithm_data["Vanilla BO"] = vanilla_data
        
        # Check for TabPFN BO data
        if Path(tabpfn_folder).exists():
            tabpfn_data = []
            for run in range(1, n_runs + 1):
            ...
            if tabpfn_data:
                available_algorithms.append("TabPFN BO")
                algorithm_data["TabPFN BO"] = tabpfn_data
        
        # Check for TabPFN Advanced BO data
        if Path(tabpfn_advanced_folder).exists():
            tabpfn_advanced_data = []
            for run in range(1, n_runs + 1):
                run_dir = Path(tabpfn_advanced_folder) / f"run_{run}"
                if not run_dir.exists():
                    continue
                ...
                if dat_files:
                    try:
                        run_data = process_dat_file(dat_files[0])
                        tabpfn_advanced_data.append(run_data)
                    except Exception as e:
                        print(f"Error processing TabPFN Advanced BO run {run}: {e}")
            
            if tabpfn_advanced_data:
                available_algorithms.append("TabPFN Advanced BO")
                algorithm_data["TabPFN Advanced BO"] = tabpfn_advanced_data
        
        # Create comparison plots if we have data from multiple algorithms
        if len(available_algorithms) >= 2:
            print(f"\nCreating algorithm comparison plot for: {', '.join(available_algorithms)}")
            
            # Create multi-algorithm comparison plot
            plot_multi_algorithm_comparison(
                algorithm_data,
                problem_id,
                dimension,
                save_path=plots_dir / f"multi_comparison_problem_{problem_id}_dim_{dimension}.png"
            )
        else:
            print("Not enough algorithm data available for comparison plot.")
